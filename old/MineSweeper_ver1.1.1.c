  ...
  for(;;){
    ...
    while(flag == 1){
      ...
      if(x>p.Coordinatex && x<1000-p.Coordinatex && y>p.Coordinatey && y<700-p.Coordinatey){
        arrayx = ClickX(x);
        arrayy = ClickY(y);
        FlagInstall(GAMEMAP,arrayx,arrayy);
      }
      if(x>10 && x<30){
         if(y>10 && y<70){
           flag = Flag(flag);
...
//表示画面
  ...
  HgSetFillColor(HG_RED);    HgBoxFill(75,400,20,20,1);
  HgSetFillColor(HG_ORANGE); HgBoxFill(75,360,20,20,1);
  HgSetFillColor(HG_YELLOW); HgBoxFill(75,320,20,20,1);
  HgSetFillColor(HG_GREEN);  HgBoxFill(75,280,20,20,1);
  HgSetFillColor(HG_SKYBLUE);HgBoxFill(75,240,20,20,1);
  HgSetFillColor(HG_BLUE);   HgBoxFill(75,200,20,20,1);
  HgSetFillColor(HG_DBLUE);  HgBoxFill(575,400,20,20,1);
...
//旗の設定
int FlagInstall(int GAMEMAP[][p.mapsizey],int arrayx,int arrayy){
  int wherex, wherey;
